...
public:
     ...
     * X_world = P X_spline
     */
    void evaluate(T t, T* &P, SE3DerivType& P_prim, SE3DerivType& P_bis);
    void evaluate(T t, T* &P) ;
...
protected:
    ...
    double offset_;  // time stamp in the very beginning
    std::vector<T*> knots_; // poses
};

template<typename T>
void UniformSpline<T>::add_knot(T *data) {
    knots_.push_back(data);
    auto p = Eigen::Map<SE3Type>(data);
    std::cout << "Add pose : " << endl << p.matrix() << endl;
}

template<typename T>
void UniformSpline<T>::evaluate(T t, T* &P, SE3DerivType &P_prim, SE3DerivType& P_bis) {

    typedef Eigen::Matrix<T, 4, 4> Mat4;
    typedef Eigen::Matrix<T, 4, 1> Vec4;
    typedef Eigen::Map<SE3Type> EigenSE3;
    ...
    EigenSE3 P0 = Eigen::Map<SE3Type>(knots_[i0]);
    EigenSE3 Pse3 = Eigen::Map<SE3Type>(P);
    ...
    for(int j : {1, 2, 3}) {
        EigenSE3 knot1 = Eigen::Map<SE3Type>(knots_[i0+j-1]);
        EigenSE3 knot2 = Eigen::Map<SE3Type>(knots_[i0+j]);
...
/// \param P : Pose
template<typename T>
void UniformSpline<T>::evaluate(T t, T* &P) {
...
//        throw std::out_of_range(ss.str());
    ...
    EigenSE3 poseOut = Eigen::Map<SE3Type>(knots_[i0]);
    std::cout << "OriPose " << endl << poseOut.matrix() << endl;

    for(int j : {1, 2, 3}) {
        EigenSE3 knot1 = Eigen::Map<SE3Type>(knots_[i0+j-1]);
        EigenSE3 knot2 = Eigen::Map<SE3Type>(knots_[i0+j]);
//        std::cout << "Knot1 " << endl << knot1.matrix() << endl;
//        std::cout << "Knot2 " << endl << knot2.matrix() << endl;

//        typename SE3Type::Tangent omega = SE3Type::log(knot1.inverse() * knot2);
//        Mat4 omega_hat = SE3Type::hat(omega);

        SE3Type Aj = SE3Type::exp(B(j) * (knot1.inverse() * knot2).log());
        poseOut *= Aj;
        A[j-1] = Aj.matrix();
...
//        A_bis[j-1] = Aj_bis;
    ...
    std::cout << "Intepolated pose at " << t << " :" << endl << poseOut.matrix() << endl;
    P = &poseOut.matrix()(0,0);
}
